...
esx:playerLoaded -> RSCore:Client:OnPlayerLoaded (use for setting a variable to let the script know the player is ready)

esx:showAdvancedNotification -> RSCore:Notify

esx:showHelpNotification -> RSCore:Notify

esx:showNotification -> RSCore:Notify

ESX.GetPlayerData -> RSCore.Functions.GetPlayerData

ESX.IsPlayerLoaded -> None (checks if player is loaded so not relevant)

ESX.SetPlayerData -> RSCore:Player:SetPlayerData

ESX.TriggerServerCallback -> RSCore.Functions.TriggerCallback

ESX.Game.DeleteObject -> None (Can use FiveM native DeleteEntity)

ESX.Game.DeleteVehicle -> RSCore.Functions.DeleteVehicle

ESX.Game.GetClosestObject -> None (Can use FiveM native GetClosestObjectOfType)

ESX.Game.GetClosestPed -> RSCore.Functions.GetClosestPed

ESX.Game.GetClosestPlayer -> RSCore.Functions.GetClosestPlayer

ESX.Game.GetClosestVehicle -> RSCore.Functions.GetClosestVehicle
...
ESX.Game.SpawnVehicle -> RSCore.Functions.SpawnVehicle

ESX.Game.Teleport -> (Can use FiveM Native SetEntityCoords and SetEntityHeading)

ESX.Game.Utils.DrawText3D -> RSCore.Functions.DrawText3D
...
local xPlayer = RSCore.Functions.GetPlayerData() <-- Client is automatically the source so no need to specify
local citizenid = xPlayer.citizenid  <-- See here i do NOT have to put "PlayerData"

--------- Playerdata:Server Side---------
local xPlayer = RSCore.Functions.GetPlayer(source) <-- Have to specify the source
...
ESX.GetPlayerFromId -> RSCore.Functions.GetPlayer
ESX.GetPlayerFromIdentifier -> RSCore.Functions.GetPlayerByCitizenId
ESX.GetPlayers -> RSCore.Functions.GetPlayers
ESX.RegisterServerCallback -> RSCore.Functions.CreateCallback
ESX.RegisterUsableItem -> RSCore.Functions.CreateUseableItem
ESX.SavePlayer -> RSCore.Player.Save
ESX.SavePlayers -> None (dont bother)
ESX.Trace -> Use RSCore.Debug but dont bother converting this
ESX.UseItem -> RSCore.Functions.UseItem 
...
xPlayer.showHelpNotification -> TriggerClientEvent('RSCore:Notify')
xPlayer.showNotification -> TriggerClientEvent('RSCore:Notify')
xPlayer.triggerEvent -> None (dont bother)
xPlayer.updateCoords -> None (dont bother) 
---------Events---------
esx:getSharedObject -> RSCore:GetObject
esx:setJob -> RSCore:Client:OnJobUpdate
esx:onPlayerSpawn -> RSCore:Client:OnPlayerLoaded
playerSpawned -> RSCore:Client:OnPlayerLoaded (spawnmanager compatibility)
esx:addInventoryItem -> RSCore:Server:AddItem
esx:removeInventoryItem -> RSCore:Server:RemoveItem
esx:useItem -> RSCore:Server:UseItem